...
const CrazyOutput = () => {
  ...
  const postCrazy = async (e) => {
    ...
    try {
      // const response = await fetch("http://localhost:8080/api/crazyEvent/registerForCrazy", {
      const response = await fetch(
        "https://server-fest.onrender.com/api/crazyEvent/registerForCrazy",
        {
          method: "POST",
          headers: {
        ...
        <div className="row justify-content-center align-items-center">
          <div className="col-lg-6">
            <h1 className="fw-bold text-center text-success mb-4">
              Crazy Event Registration
            </h1>
            <form onSubmit={postCrazy}>
              <div className="mb-3">
              ...
              <div className="mb-3">
                ...
                <div className="form-check">
                  <input
                    ...
                    type="checkbox"
                    name="teamBuildingBaloonGame"
                    checked={formData.interestedGames.teamBuildingBaloonGame}
                    onChange={handleChange}
                  />
                  <label className="form-check-label">
                    Team Building Ballon Game
                  </label>
                </div>
                <div className="form-check">
                  <input
                    ...
                    type="checkbox"
                    name="miniBasket"
                    checked={formData.interestedGames.miniBasket}
                    onChange={handleChange}
                  />
                  <label className="form-check-label">Mini Basket</label>
                </div>
                <div className="form-check">
                  <input
                    className="form-check-input"
                    type="checkbox"
                    name="ticTacToe"
                    checked={formData.interestedGames.ticTacToe}
                    onChange={handleChange}
                  />
                  <label className="form-check-label">Tic Tac Toe</label>
                </div>
              </div>
